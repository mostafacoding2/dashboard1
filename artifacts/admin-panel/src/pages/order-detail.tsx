...
import { ArrowRight, Truck, CheckCircle2, Clock, XCircle, CreditCard, MapPin, User, Package, Image, Copy, Phone, Mail, MessageCircle, MessageSquare, Pencil, Trash2, Download, Share2, Printer, Maximize2, AlertTriangle, FileImage } from "lucide-react";
...
export default function OrderDetail() {
  ...
  const navStatus = new URLSearchParams(window.location.search).get("status");
  ...
  return (
    <div className="min-h-screen bg-muted/30">
      ...
      <div className="py-3 px-4 space-y-3">
        {/* Shipping Manifest - Completed Orders */}
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {manifestImage && (
                      <>
                        ...
                        <Button variant="ghost" size="sm" className="h-6 w-6 p-0 text-white hover:bg-white/20" onClick={() => {
                          if (navigator.share) {
                            fetch(manifestImage).then(r => r.blob()).then(blob => {
                              const file = new File([blob], manifestFileName || `manifest-${order.id}.png`, { type: "image/png" });
                              navigator.share({ files: [file], title: `بوليصة الشحن - طلب #${order.id}` });
                            });
                          }
                        }}>
              ...
              <CardContent className="p-4">
                {manifestImage ? (
                  ...
                  <div className="text-center py-8">
                    ...
                    <p className="text-xs text-muted-foreground/70 mb-3">ارفع صورة بوليصة الشحن لإتمام الطلب</p>
      ...
      <Dialog open={editingManifest} onOpenChange={() => setEditingManifest(false)}>
        <DialogContent className="max-w-sm">
          ...
          <DialogFooter>
            ...
            <Button size="sm" onClick={() => setEditingManifest(false)}>حفظ</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
